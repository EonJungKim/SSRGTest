...
import com.ppsoln.ssrgdecoder.dataType.*;
...
public class SsrgDecoder {
    ...
    public Ssr ssrgParser() {
        ...
        for (RtcmLine line : rtcmLine) {
            if ( line.no == 4090) {
                // no = Util.readUBits(line.buf, 0, 12);
                // subType = Util.readUBits(line.buf, 12, 4);
    ...
    public void parseSM001(String buf) {
        ...
        // int updateInterval = Util.readUBits(buf, 47, 4);
        // int multipleMessageIndicator = Util.readUBits(buf, 51, 1);
        // int reserved = Util.readUBits(buf, 53, 4);
